...
for (let i = 0; i < starWarsCharacters.length; i++) {
  const characObj = starWarsCharacters[i];
  const name = characObj.name;
...
while (index < starWarsCharacters.length) {
  crewMass += starWarsCharacters[index].mass || 0;
  index++;
}
console.log("Massa totale equip", crewMass);
/* ESERCIZIO 7
...
for (let i = 0; i < starWarsCharacters.length; i++) {
  if (starWarsCharacters[i].gender === "n/a") {
    ...
    robotUpdate.push(starWarsCharacters[i]);
  }
}
